...
服务启动时会拉起一个浏览器窗口并访问上游站点。窗口默认**最小化到任务栏、不抢焦点**（平时不会弹到前台）。

> **多数情况下你什么都不用做** —— 只要浏览器里之前留有有效的 `cf_clearance` cookie，
> 网关就能直接工作。需要人机验证或采集 Turnstile 时，窗口会**自动呼到前台**；完成后会再最小化回去。任务栏图标随时可以点开。
...
- `"session_ready": false` → 看 `hint` 字段的说明：
  - 若窗口显示验证页 → 点任务栏图标打开窗口，手动过一次即可
  - 若窗口是正常页面 → 通常是上游限流或临时异常，**稍后重试即可，不必反复点验证**

> 这个窗口**需要保持开启**（最小化即可，不要关掉）。关闭窗口 → 网关返回 503。
> 也可双击 `minimize_window.bat` 手动最小化。

> 补充：窗口里会常驻 **两个标签页**——
> - ` /chat/gpt-5-mini`：聊天
> - ` /image-generators/gpt-image-2`：图生图 / Turnstile
>
> 实测：两个页都能打同源 API，但 **Turnstile 只在工作台页能稳定自动签发**；停在 chat 页采 token 经常失败，
> 那时才需要你去点 Cloudflare。改图走官方工作台链路
> （`moderate-image` → `upload-photo` → `/api/v2/generate-image`），token 在工作台页自动采集。
...
| 3 | **后台标签 / 最小化窗口会被节流** | 常规聊天请求不再激活窗口。Turnstile 采集和 Cloudflare 挑战页时才会把窗口呼到前台（否则 token 签不出来）。 |
...
A：Turnstile token 采集失败。网关会自动把窗口呼到前台再采；若仍失败，点任务栏打开窗口确认页面正常，或配置 `TURNSTILE_SOLVER_URL`。

**Q：改图返回 503 `at capacity`？**
A：这是上游工作台容量满了，不是本机窗口挂了。换模型或稍后再试。

**Q：启动后浏览器在任务栏但点不开？**
A：旧版本曾把窗口放到屏幕外。当前版本会把窗口摆回屏幕内再最小化，点任务栏即可恢复。若仍异常，运行 `minimize_window.bat` 后再点一次。
